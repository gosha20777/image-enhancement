...
class DPED(object):
    def __init__(self, sess, config, dataset_phone, dataset_dslr):
        ...
        self.phone_patch = tf.compat.v1.placeholder(tf.compat.v1.float32, [self.batch_size, self.patch_size, self.patch_size, self.channels], name='input_phone_patch') 
        self.dslr_patch = tf.compat.v1.placeholder(tf.compat.v1.float32, [self.batch_size, self.patch_size, self.patch_size, self.channels], name='input_dslr_patch') 
        
        # images for testing (unknown size)
        self.phone_test = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, self.patch_size, self.patch_size, self.channels], name='input_phone_test')
        self.phone_test_unknown = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, None, None, self.channels], name='input_phone_test_unknown_size')
        self.dslr_test = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, self.patch_size, self.patch_size, self.channels], name='input_dslr_test')
        
        # input to discriminator network
        self.input_discriminator = tf.compat.v1.placeholder(tf.compat.v1.float32, [self.batch_size, self.patch_size, self.patch_size, self.channels], name='input_discriminator') 
    ...
    def generator_network(self, image):
        with tf.compat.v1.variable_scope('generator', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            b1_in = tf.compat.v1.layers.conv2d(image, 64, 9, strides = 1, padding = 'SAME', name = 'CONV_1', kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
            b1_in = tf.compat.v1.nn.relu(b1_in)
            ...
            temp = tf.compat.v1.layers.conv2d(b4_out, 64, 3, strides = 1, padding = 'SAME', name = 'CONV_2', kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
            temp = tf.compat.v1.nn.relu(temp)
            temp = tf.compat.v1.layers.conv2d(temp, 64, 3, strides = 1, padding = 'SAME', name = 'CONV_3', kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
            temp = tf.compat.v1.nn.relu(temp)
            temp = tf.compat.v1.layers.conv2d(temp, 64, 3, strides = 1, padding = 'SAME', name = 'CONV_4', kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
            temp = tf.compat.v1.nn.relu(temp)
            temp = tf.compat.v1.layers.conv2d(temp, 3, 1, strides = 1, padding = 'SAME', name = 'CONV_5', kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
            return temp
    
    def resblock(self, feature_in, num):
        # subblock (conv. + BN + relu)
        temp =  tf.compat.v1.layers.conv2d(feature_in, 64, 3, strides = 1, padding = 'SAME', name = ('resblock_%d_CONV_1' %num), kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
        #temp = tf.compat.v1.layers.batch_normalization(temp, name = ('resblock_%d_BN_1' %num))
        temp = tf.compat.v1.nn.relu(temp)
        
        # subblock (conv. + BN + relu)
        temp =  tf.compat.v1.layers.conv2d(temp, 64, 3, strides = 1, padding = 'SAME', name = ('resblock_%d_CONV_2' %num), kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
        #temp = tf.compat.v1.layers.batch_normalization(temp, name = ('resblock_%d_BN_2' %num))
        temp = tf.compat.v1.nn.relu(temp)
    ...
    def build_generator_loss(self):
        ...
        self.color_loss = 255 * tf.compat.v1.reduce_mean(tf.compat.v1.square(gaussian_blur(self.dslr_patch)-gaussian_blur(self.enhanced_patch)))
        
        # texture loss (gan)
        self.texture_loss = tf.compat.v1.reduce_mean(sigmoid_cross_entropy_with_logits(self.logits_dslr, self.logits_enhanced))
        
        # content loss (vgg)
        enhanced_vgg = net(self.vgg_dir, self.enhanced_patch * 255)
        dslr_vgg = net(self.vgg_dir, self.dslr_patch * 255)
        self.content_loss = tf.compat.v1.reduce_mean(tf.compat.v1.square(enhanced_vgg[self.content_layer] - dslr_vgg[self.content_layer])) 
        
        # tv loss (tv)
        self.tv_loss = tf.compat.v1.reduce_mean(tf.compat.v1.image.total_variation(self.enhanced_patch))
        ...
        self.G_optimizer = tf.compat.v1.train.AdamOptimizer(self.config.learning_rate).minimize(self.G_loss, var_list=self.g_var)
    ...
    def build_discriminator(self):
        ...
        d_loss_real = tf.compat.v1.reduce_mean(sigmoid_cross_entropy_with_logits(self.logits_dslr, tf.compat.v1.ones_like(self.logits_dslr)))
        d_loss_fake = tf.compat.v1.reduce_mean(sigmoid_cross_entropy_with_logits(self.logits_phone, tf.compat.v1.zeros_like(self.logits_phone)))
        
        self.d_loss = d_loss_real + d_loss_fake
        self.D_optimizer = tf.compat.v1.train.AdamOptimizer(self.config.learning_rate).minimize(self.d_loss, var_list=self.d_var)
        
    def discriminator_network(self, image):
        with tf.compat.v1.variable_scope('discriminator', reuse = tf.compat.v1.AUTO_REUSE):
            #convert to grayscale image
            image_gray = tf.compat.v1.image.rgb_to_grayscale(image)
            
            # conv layer 1 
            temp = tf.compat.v1.layers.conv2d(image_gray, 48, 11, strides = 4, padding = 'SAME', name = 'CONV_1', kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
            temp = lrelu(temp)
            
            # conv layer 2
            temp = tf.compat.v1.layers.conv2d(temp, 128, 5, strides = 2, padding = 'SAME', name = 'CONV_2', kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
            temp = tf.compat.v1.layers.batch_normalization(temp, name = 'BN_2')
            temp = lrelu(temp)
            
            # conv layer 3
            temp = tf.compat.v1.layers.conv2d(temp, 192, 3, strides = 1, padding = 'SAME', name = 'CONV_3', kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
            temp = tf.compat.v1.layers.batch_normalization(temp, name = 'BN_3')
            temp = lrelu(temp)
            
            # conv layer 4
            temp = tf.compat.v1.layers.conv2d(temp, 192, 3, strides = 1, padding = 'SAME', name = 'CONV_4', kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
            temp = tf.compat.v1.layers.batch_normalization(temp, name = 'BN_4')
            temp = lrelu(temp)
            
            # conv layer 5
            temp = tf.compat.v1.layers.conv2d(temp, 128, 3, strides = 2, padding = 'SAME', name = 'CONV_5', kernel_initializer = tf.compat.v1.initializers.glorot_uniform(), reuse=tf.compat.v1.AUTO_REUSE)
            temp = tf.compat.v1.layers.batch_normalization(temp, name = 'BN_5')
            temp = lrelu(temp)
            
            # FC layer 1
            fc_in = tf.compat.v1.layers.flatten(temp)
            fc_out = tf.compat.v1.layers.dense(fc_in, units = 1024, activation = None)
            fc_out = lrelu(fc_out)
            
            # FC layer 2
            logits = tf.compat.v1.layers.dense(fc_out, units = 1, activation = None)
    ...
    def test_discriminator(self, test_num, load = False, mode = "phone_dslr"):
        ...
        for i in range(test_num):
            ...
            test_patch_dslr = preprocess(cv2.imread(test_list_dslr[index]).astype("float32"))
            prob = self.sess.run(self.prob, feed_dict={self.phone_test: [test_patch_dslr]})
            if prob > 0.5:
                probs[i] = prob
                acc_dslr += 1
            test_patch_phone = preprocess(cv2.imread(test_list_phone[index]).astype("float32"))
    ...
    def test_generator(self, test_num_patch = 200, test_num_image = 5, load = False):
        ...
        for i in range(test_num_patch):
            ...
            test_patch_enhanced = self.sess.run(self.enhanced_test , feed_dict={self.phone_test:[test_patch_phone], self.dslr_test:[test_patch_dslr]})
            if i % 50 == 0:
                cv2.imwrite(("./samples/%s/patch/phone_%d.png" %(self.config.dataset_name, i)), postprocess(test_patch_phone))
                cv2.imwrite(("./samples/%s/patch/dslr_%d.png" %(self.config.dataset_name,i)), postprocess(test_patch_dslr))
                cv2.imwrite(("./samples/%s/patch/enhanced_%d.png" %(self.config.dataset_name,i)), postprocess(test_patch_enhanced[0]))
    ...
    def build_log_summary(self):  
        ...
        self.loss_summary = tf.compat.v1.summary.scalar("loss", self.G_loss)
        self.summary_merged = tf.compat.v1.summary.merge([self.output_summary, self.loss_summary])
